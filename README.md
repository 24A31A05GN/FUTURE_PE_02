# FUTURE_PE_02

## Task 2: AI Content Marketing using UGC Ads

### Internship
Future Interns – Prompt Engineering Internship

### Product Chosen
Boat Airdopes Earbuds

### Objective
Generate AI-powered UGC-style marketing content including hooks, ad scripts, CTAs, and social media captions.

### Deliverables
- UGC Ad Hooks
- Ad Scripts
- Social Media Captions
- Prompt Documentation

### Files Included

| File | Description |
|------|-------------|
| README.md | Project documentation |
| prompts.md | Prompt used |
| hooks.md | UGC ad hooks |
| ad-scripts.md | Ad scripts |
| captions.md | Social media captions |

### Tools Used
- ChatGPT
- GitHub

### Skills Demonstrated
- Prompt Engineering
- Content Marketing
- UGC Advertising
- Copywriting
- AI Content Creation

### Outcome
Successfully generated a complete AI-powered UGC Ad Content Pack for Boat Airdopes Earbuds.
